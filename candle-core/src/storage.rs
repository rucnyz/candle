...
use crate::{
    CpuStorage, CudaStorage, CustomStorage, DType, Device, Error, Layout, MetalStorage, Result,
    Shape,
};
...
impl Storage {
    ...
    pub(crate) fn const_set(&mut self, v: Scalar, l: &Layout) -> Result<()> {
        match self {
            ...
            Storage::Custom(storage) => storage.const_set(v, l),
        }
    }
    ...
    pub(crate) fn apply_op1(&self, l: &Layout, c: &dyn CustomOp1) -> Result<(Self, Shape)> {
        match self {
            ...
            Self::Custom(_) => {
                crate::bail!("custom backend: custom_op not supported, use TensorHook")
            }
        }
    }
    ...
    pub(crate) fn apply_op3(
        ...
        match (self, t2, t3) {
            ...
            (Self::Custom(_), Self::Custom(_), Self::Custom(_)) => {
                crate::bail!("custom backend: custom_op not supported, use TensorHook")
            }
            _ => unreachable!(),
        }
