...
impl Tensor {
    ...
    /// Returns a mutable reference to the storage and the layout.
    ///
    /// # Safety
    /// The caller must ensure that no other references to this tensor's storage
    /// are used concurrently and that any modifications maintain the tensor's
    /// dtype and size invariants.
